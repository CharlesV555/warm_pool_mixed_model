...
OUTPUT_PATH = EXAMPLES_DIR / "cross_catalysis_SSA_linear_trajectory.npz"
FAST_NETWORK_REPORT_PATH = EXAMPLES_DIR / "cross_catalysis_SSA_linear_fast_network_report.txt"

# Set this to False to disable the finite-Markov fast-subnetwork diagnostic.
# The report is observational only: it does not change the SSA stepper state,
# partition, or propensities.
fast_network_report = False # pdmp用来看partition的，ssa不需要
...
def main() -> None:
    ...
    stepper = SSAStepper()  # Use SSAStepper for the base stepper
    ...
    print(f"ssa network n_species={network.n_species}, n_channels={network.n_channels}")
    ...
    trajectory_record.run_metadata["ssa_network"] = {
